...
Т.е первый блок server слушает hhtp-запросы на порту 80, отвечает на запросы к домену и перенаправляет все http-запросы на https. Второй блок server слушает hhtp-запросы на порту 443, указывает пути к SSl-сертификату и ключц, определяет корневую директорию и обрабатывает все запросы к сайту
